...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin){
    ...
    // current_tick_1�?? old_tick_1?�� 차이�?? 계산?�� ?�� ?��버플로우�?? 고려
    int32_t diff_1 = (int32_t)(current_tick_1 - old_tick_1);
    if (diff_1 < 0) {  // ?��버플로우�?? 발생?�� 경우
        diff_1 += UINT32_MAX + 1;  // 최�?값을 ?��?��?�� 차이�?? 계산
    }
    ...
    // current_tick_2?? old_tick_2?�� 차이�?? 계산?�� ?�� ?��버플로우�?? 고려
    int32_t diff_2 = (int32_t)(current_tick_2 - old_tick_2);
    if (diff_2 < 0) {  // ?��버플로우�?? 발생?�� 경우
        diff_2 += UINT32_MAX + 1;  // 최�?값을 ?��?��?�� 차이�?? 계산
    }
    ...
    if (diff_3 < 0) {  // ?��버플로우�?? 발생?�� 경우
        diff_3 += UINT32_MAX + 1;  // 최�?값을 ?��?��?�� 차이�?? 계산
    }

    if(GPIO_Pin == han3_Pin && diff_3 > 200) {
        state_2++;
