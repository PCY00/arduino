...
// wind degree get
  for(int i =0;i<16;i++){
    if(direct[i]-0.02 < volt && direct[i]+0.02 > volt){
       float degree = i * 22.5;
       break;
    }
  }
//==================================================================================
//SO2, NO2 data get
  while (SO2_serial.available()) { // read from AGSM port, send to Serial port to interupt continuous output send 'c''/r' without line ending, may have to send more than once.
